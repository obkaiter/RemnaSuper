    ...
    while true; do
        ...
        case $choice in
            ...
            2) run_action "Поиск стратегии ss-zapret" \
                "Будут проверены компоненты blockcheck, обработка zapret будет временно остановлена, затем запустится интерактивный поиск стратегий nfqws без TPWS. После завершения или прерывания контейнер будет перезапущен." \
